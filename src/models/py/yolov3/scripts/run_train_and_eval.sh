...
OUTPUT_DIR="${PROJECT_ROOT}/src/models/py/yolov3/model_outputs/${RUN_NAME}"
mkdir -p "${OUTPUT_DIR}"
...
if [ ! -f "${DARKNET53_WEIGHTS}" ]; then
    ...
    ./src/models/py/yolov3/scripts/download_darknet_weights.sh
else
...
if [ "$1" == "--eval" ]; then
    ...
    python -m src.models.py.yolov3.evaluate \
