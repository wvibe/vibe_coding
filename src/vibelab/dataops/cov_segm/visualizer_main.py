...
# Use full import paths from vibelab
from vibelab.dataops.cov_segm.datamodel import SegmSample
from vibelab.dataops.cov_segm.loader import (
    load_sample,
)
from vibelab.dataops.cov_segm.visualizer import visualize_prompt_masks
...
def _setup_argparse() -> argparse.ArgumentParser:
    ...
    group_dataset.add_argument(
        "--start-index", type=int, default=0, help="Starting index of the sample in the dataset."
    )
    group_dataset.add_argument(
        "--sample-count",
        type=int,
...
def _configure_logging(debug_mode: bool):
    ...
    if debug_mode:
        ...
        logging.getLogger("vibelab.dataops.cov_segm.loader").setLevel(logging.DEBUG)
        logging.getLogger("vibelab.dataops.cov_segm.visualizer").setLevel(logging.DEBUG)
