...
def _get_batch_image_ids(
    years: List[str], tags: List[str], voc_root: Path
    ...
    for year in years:
        for tag in tags:
            try:
                ...
                imageset_path = get_image_set_path(voc_dir, "detect", tag)
                image_ids = read_image_ids(imageset_path)  # Use the util
                if image_ids:
                    all_ids_found.extend([(img_id, year, tag) for img_id in image_ids])
                else:
                    logger.warning(f"No image IDs found for {year}/{tag} in {imageset_path}")
            except FileNotFoundError:
                logger.error(f"Could not find ImageSet file for {year}/{tag}")
            except Exception as e:
                logger.error(f"Error reading ImageSet for {year}/{tag}: {e}")
    return all_ids_found


def get_target_image_list(args: argparse.Namespace, voc_root: Path) -> List[Tuple[str, str, str]]:
    ...
    if args.image_id:
        ...
        # Basic path check
        try:
            voc_dir = get_voc_dir(voc_root, first_year)
            img_path = get_image_path(voc_dir, args.image_id)
            xml_path = get_annotation_path(voc_dir, args.image_id)
        ...
        all_ids_found = _get_batch_image_ids(years, tags, voc_root)

        if not all_ids_found:
            logger.error("No image IDs found for any specified year/tag combination.")
            return []

        logger.info(f"Found {len(all_ids_found)} total image IDs across specified splits.")
...
def main():
    ...
    # --- Path Setup ---
    voc_root_str = args.voc_root or os.getenv("VOC_ROOT")
    if not voc_root_str:
    ...
    if not (voc_root / "VOCdevkit").exists():
        logger.error(f"VOCdevkit not found in specified VOC root: {voc_root}")
        return

    output_root = Path(args.output_root or voc_root_str).expanduser()
    output_dir = output_root / args.output_subdir
    # Always create the output directory structure if saving might occur
    output_dir.mkdir(parents=True, exist_ok=True)
    logger.info(f"Output visualization directory: {output_dir}")

    # --- Identify Images ---
    ids_to_process = get_target_image_list(args, voc_root)
