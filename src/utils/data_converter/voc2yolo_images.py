...
def copy_images_for_split(voc_root: Path, output_root: Path, year: str, tag: str):
    """Copies images for a specific year and tag.
    ...
    try:
        ...
        return 0, 0  # Indicate no attempt was made due to setup failure
    except ValueError as e:
    ...
    logger.info(f"Found {len(image_ids)} image IDs. Copying images...")
    ...
    logger.info(
        f"Finished copying for {year}/{tag}: Copied {success_count}, Failed/Skipped {fail_count}"
    )
...
def main():
    ...
    for year in years:
        for tag in tags:
            try:
                s, f = copy_images_for_split(voc_root, output_root, year, tag)
                total_success += s
                total_fail += f
            except Exception as e:
                logger.error(
