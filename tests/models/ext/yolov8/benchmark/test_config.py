...
def test_load_valid_config(tmp_path):
    ...
    try:
        ...
        test_annotations_dir = tmp_path / "labels"
        test_images_dir.mkdir()
        test_annotations_dir.mkdir()

        # Update paths in config_data to use temporary directories
        config_data["dataset"]["test_images_dir"] = str(test_images_dir)
        config_data["dataset"]["annotations_dir"] = str(test_annotations_dir)
        ...
        assert config.metrics.object_size_definitions.large[1] == float(
            "inf"
        )  # Check inf conversion
